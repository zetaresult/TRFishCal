...
if menu == "경험치 및 낚시 계산기":
    ...
    st.markdown(f"<div style='font-size: 20px; font-weight: bold; margin-top: 12px;'>보정 후: {selected_name} {round((rod_seconds/((min_default2+max_default2)/2))*1.7):,}개는 {round(selected_exp * ((rod_seconds/((min_default2+max_default2)/2)))*1.7):,}EXP입니다.</div>", unsafe_allow_html=True)
